...
'''
AugmentedLSTM: similar to the univariate multistep LSTM, 
this architecture augments the LSTM output with the last few points (nAugmentLength)
'''

class AugmentedLSTM(nn.Module):
    def __init__(self, timesteps, features, nAugmentLength=100, lstm_units=64, forecast_length=1, nn_activation = 'linear'):
        """
        LSTM Model for univariate time series with multi-step one-shot prediction.
        ...
        - lstm_units (int): Number of units in the LSTM layer.
        - forecast_length (int): Number of steps to predict into the future (in one shot).
        - nAugmentLength (int): Number of timesteps to take raw values from
        """
        ...
        self.features = features
        self.lstm_units = lstm_units
        self.nAugmentLength = nAugmentLength
        assert nn_activation == 'relu' or nn_activation == 'linear' #only supports linear or relu now
        self.nn_activation = nn_activation
        self.forecast_length = forecast_length  # Number of timesteps to predict
        ...
        self.output = nn.Linear(lstm_units + self.nAugmentLength, forecast_length * features)
    ...
    def forward(self, x):
        ...
        augmentValues = torch.squeeze(x[:,-self.nAugmentLength:,:],dim=2) #last timestep values we take to augment LSTM ouput
        #shape: (batch_size,self.nAugmentLength)

        # Use the output from the last timestep of the LSTM
        last_timestep_output = lstm_out[:, -1, :]  # shape: (batch_size, lstm_units)

        assert augmentValues.shape[0] == last_timestep_output.shape[0]
        augmented_output = torch.cat((last_timestep_output,augmentValues), dim=1)

        # Pass through the fully connected layer to predict the full forecast
        if self.nn_activation == 'relu':
            fc_output = F.relu(self.output(augmented_output))  # shape: (batch_size, forecast_length * features)
        else:
            fc_output = self.output(augmented_output)
